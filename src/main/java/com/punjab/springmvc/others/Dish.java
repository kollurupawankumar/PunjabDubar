...
public class Dish {
  ...
  public static List<Dish> all() {
    try(Connection con = DB.sql2o.open()) {
      String sql = "SELECT id AS mId, name AS mName, category AS mCategory from dumdar.dishes ORDER BY category, name";
  ...
  public void save() {
    try(Connection con = DB.sql2o.open()) {
      String sql = "INSERT INTO dumdar.dishes(name, category) VALUES (:name, :category)";
  ...
  public static Dish find(int id) {
    try(Connection con = DB.sql2o.open()) {
      String sql = "SELECT id AS mId, name AS mName, category AS mCategory from dumdar.dishes WHERE id = :id";
  ...
  public void delete() {
    try(Connection con = DB.sql2o.open()) {
      String sqlDeleteDishConnection = "DELETE from dumdar.dishes_ingredients WHERE dishes_ingredients.dish_id = :id;";
      String sqlDeleteDish = "DELETE from dumdar.dishes WHERE id = :id;";
  ...
  public void update(String newName, int newCategory) {
    ...
    try(Connection con = DB.sql2o.open()) {
      String sql = "UPDATE dumdar.dishes SET name = :name, category = :category WHERE id = :id";
      con.createQuery(sql)
  ...
  public void addIngredient(int ingredientId, int ingredientAmount) {
    try(Connection con = DB.sql2o.open()) {
      String sql = "INSERT INTO dumdar.dishes_ingredients (dish_id, ingredient_id, ingredient_amount) VALUES (:dishId, :ingredientId, :ingredientAmount)";
      con.createQuery(sql)
  ...
  public void removeIngredient(int ingredientId) {
    try(Connection con = DB.sql2o.open()) {
      String sql = "DELETE from dumdar.dishes_ingredients WHERE dishes_ingredients.ingredient_id = :id AND dishes_ingredients.dish_id = :dishId";
      con.createQuery(sql)
  ...
  public List<Recipe> getAllRecipes() {
    try(Connection con = DB.sql2o.open()) {
      String sql = "SELECT id AS mId, dish_id AS mDishId, ingredient_id AS mIngredientId, ingredient_amount AS mIngredientAmount from dumdar.dishes_ingredients WHERE dish_id = :dishid";
      return con.createQuery(sql)
                .addParameter("dishid", mId)
  ...
  public int getTimesOrderedToday() {
    try (Connection con = DB.sql2o.open()) {
      String sql = "SELECT COUNT(orders.id) from dumdar.orders WHERE dish_id = :dishid AND orders.creation_date = to_date(:creationdate, 'YYYY-MM-DD')";
      return con.createQuery(sql)
        .addParameter("dishid", mId)
