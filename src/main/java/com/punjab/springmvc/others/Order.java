...
public class Order {
  ...
  public static List<Order> getAllActive() {
    try (Connection con = DB.sql2o.open()) {
      String sql = "SELECT id as mId, patron_id AS mPatronId, table_num AS mTable, seat_num AS mSeat, dish_id AS mDishId, comments AS mComments, creation_date AS mCreationDate, creation_time AS mCreationTime, completion_date AS mCompletionDate, completion_time AS mCompletionTime, is_paid AS mPaid, is_up AS mUp FROM dumdar.orders WHERE completion_time IS NULL ORDER BY table_num, creation_date, creation_time";
      return con.createQuery(sql)
        .executeAndFetch(Order.class);
    }
  }
  ...
  public void addComments(String comments) {
    mComments = comments;
    try (Connection con = DB.sql2o.open()) {
      String sql = "UPDATE dumdar.orders SET comments = :comments WHERE id = :id";
      con.createQuery(sql)
  ...
  public void pay() {
    mPaid = true;
    try (Connection con = DB.sql2o.open()) {
      String sql = "UPDATE dumdar.orders SET is_paid = true WHERE id = :id";
      con.createQuery(sql)
        .addParameter("id", mId)
        .executeUpdate();
    }
  ...
  public void setIsUp() {
    mUp = true;
    try (Connection con = DB.sql2o.open()) {
      String sql = "UPDATE dumdar.orders SET is_up = true WHERE id = :id";
      con.createQuery(sql)
        .addParameter("id", mId)
        .executeUpdate();
    }
  ...
  public void changeDish(int newDishId) {
    mDishId = newDishId;
    try (Connection con = DB.sql2o.open()) {
      String sql = "UPDATE dumdar.orders SET dish_id = :newdish WHERE id = :id";
      con.createQuery(sql)
  ...
  public void complete() {
    ...
    try (Connection con = DB.sql2o.open()) {
      String sql = "UPDATE dumdar.orders SET completion_date = to_date(:completiondate, 'YYYY-MM-DD'), completion_time = :completiontime WHERE id = :id";
      con.createQuery(sql)
  ...
  public void save() {
    try (Connection con = DB.sql2o.open()) {
      String sql = "INSERT INTO dumdar.orders (patron_id, table_num, seat_num, dish_id, creation_date, creation_time, comments) VALUES (:patronid, :table, :seat, :dish, to_date(:creationdate, 'YYYY-MM-DD'), :creationtime, :comments)";
  ...
  public void make() {
    ...
    for (Ingredient ingredient : theseIngredients) {
      try (Connection con = DB.sql2o.open()) {
        String sql = "SELECT ingredient_amount FROM dumdar.dishes_ingredients WHERE dish_id = :dishid AND ingredient_id = :ingredientid";
  ...
  public static int getTotalOrdersForToday() {
    try (Connection con = DB.sql2o.open()) {
      String sql = "SELECT COUNT(orders.id) FROM dumdar.orders WHERE orders.creation_date = to_date(:creationdate, 'YYYY-MM-DD')";
      return con.createQuery(sql)
